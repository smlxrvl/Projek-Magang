<?php
    ...
    $title = "Rekap Presensi"; // Title untuk halaman peserta
    ...
    $id_peserta = $_SESSION['id'] ?? null; // Ambil ID peserta dari sesi

    // Gunakan prepared statement untuk query utama
    $query = "SELECT presensi.*, peserta.nama, peserta.lokasi_presensi FROM presensi JOIN peserta ON presensi.id_peserta = peserta.id WHERE presensi.id_peserta = ?";
    $params = [$id_peserta];
    $types = "i"; // 'i' untuk integer (id_peserta)

    if(!empty($tanggal_dari) && !empty($tanggal_sampai)) {
        // Jika kedua tanggal diisi, filter berdasarkan rentang
        $query .= " AND tanggal_datang BETWEEN ? AND ?";
        $params[] = $tanggal_dari;
        $params[] = $tanggal_sampai;
        $types .= "ss"; // Tambahkan 'ss' untuk dua string tanggal
    }
    ...
    if ($stmt) {
        ...
        // Anda bisa redirect atau menampilkan pesan error di sini
        $result = mysqli_query($connection, "SELECT presensi.*, peserta.nama, peserta.lokasi_presensi FROM presensi JOIN peserta ON presensi.id_peserta = peserta.id WHERE 1=0"); // Query kosong agar tidak ada data
    }

    // Ambil jam masuk kantor untuk perhitungan keterlambatan
    $jam_datang_ktr_global = '00:00:00'; // Default
    $lokasi_presensi_session = $_SESSION['lokasi_presensi'] ?? '';
    ...
    <div class="container-xl">
        ...
        <?php } else { ?>
            ...
            <div class="row">
                <div class="col-md-2 col-12 mb-3">
                    ...
                    data-bs-target="#exportModal"> <!-- ID modal disesuaikan -->
        ...
        <div class="table-responsive">
            ...
            <?php $no = 1; while($rekap = mysqli_fetch_array($result)) : 
                ...
                // TOTAL JAM TERLAMBAT //
                // Gunakan $jam_datang_ktr_global yang sudah diambil di awal script
                $jam_datang_real = date('H:i:s', strtotime($rekap['jam_datang']));
                $timestamp_masuk_real = strtotime($jam_datang_real);
                $timestamp_jam_datang_ktr = strtotime($jam_datang_ktr_global); // Menggunakan variabel global
        ...
        <div class="modal" id="exportModal" tabindex="-1"> <!-- ID modal disesuaikan -->
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Export Rekap Presensi To Excel</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="exportForm" method="POST" action="<?= base_url('peserta/presensi/export.php') ?>">
                        <input type="hidden" name="id_peserta" value="<?= htmlspecialchars($id_peserta) ?>">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" id="exportButton" class="btn btn-primary">Export</button> <!-- ID disesuaikan -->
                        </div>
                    </form>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Event listener untuk modal export
    const exportButton = document.getElementById('exportButton'); // ID disesuaikan
    if (exportButton) {
        exportButton.addEventListener('click', function() {
            ...
            const exportForm = document.getElementById('exportForm'); // ID disesuaikan

            if (tanggalDari === "" || tanggalSampai === "") {
                // Tampilkan SweetAlert jika field kosong
                Swal.fire({
                    icon: 'warning',
                    title: 'Peringatan!',
                ...
                if (dateDari > dateSampai) {
                    // Tampilkan SweetAlert jika tanggal awal lebih besar dari tanggal akhir
                    Swal.fire({
                        icon: 'error', // Menggunakan icon 'error' untuk kesalahan logika
                    ...
                    const exportModalInstance = bootstrap.Modal.getInstance(document.getElementById('exportModal')); // ID disesuaikan
